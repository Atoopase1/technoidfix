...
// Same table applies per phase for three-phase circuits
const VD_MV_AM = {
...
// MCB In must not exceed cable Iz (IEC 60364-5-52 Table B.52.3, conduit at 30°C).
// 32A needs 10mm² minimum — 6mm² tops out at 25A in conduit.
const CABLE_MCB_MAP = {
...
// Applies when circuits share conduit. More circuits = lower factor.
...
// Motor cable sized at 1.25× FLC (IEC 60364-4-43). Resistive loads use direct current.
// DOL inrush 6-8× FLC — Curve C handles up to 10× In so it's fine.
...
function renderLoadSelection() {
  ...
  <div class="sc">
    <div class="sc-hdr">
      <div class="ic"><i class="fas fa-plug"></i></div>
    ...
    <div class="sc-body">
      ...
      <div class="load-section">
        <h3><i class="fas fa-lightbulb"></i> LIGHTING LOADS</h3>
        <div class="fg fg-4">
      ...
      <!-- SOCKETS -->
      <div class="load-section">
        <h3><i class="fas fa-plug"></i> SOCKET OUTLETS</h3>
        <div class="fg fg-4">
      ...
      <!-- HEAVY APPLIANCES -->
      <div class="load-section">
        <h3><i class="fas fa-bolt"></i> HEAVY APPLIANCES</h3>
        <div class="fg fg-3">
...
function buildCircuits(room, voltage, installType, temp, circuitsInConduit) {
  ...
  function selectCableAndMCB(amps, isMotor, forceCable, forceMCB) {
    // Only motor loads use 1.25× multiplier for cable sizing.
  ...
  const makeCircuit = (name, type, watts, maxVD, forceCable, forceMCB, lightPoints) => {
    ...
    // Total VD = feeder VD (meter→DB) + final circuit VD (DB→point of use)
...
function renderResults() {
  ...
  setTimeout(() => {
    ...
    const warnHTML = R.warnings.map(w =>
      w.type === 'error'
        ? `<div class="error-box"><span class="wi"><i class="fas fa-times-circle"></i></span><p>${w.msg}</p></div>`
        : `<div class="warn-box"><span class="wi"><i class="fas fa-exclamation-triangle"></i></span><p>${w.msg}</p></div>`
    ).join('');

    const okHTML = R.warnings.length === 0
      ? `<div class="ok-box"><span class="wi"><i class="fas fa-check-circle"></i></span><p>All circuits comply with IEC 60364. No undersized cables or protection devices detected.</p></div>`
      : '';

    c.innerHTML = `
    <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:18px;flex-wrap:wrap;gap:10px">
      <h2 style="font-family:'Orbitron',monospace;font-size:clamp(.75rem,3vw,1rem);color:var(--el);line-height:1.3;min-width:0"><i class="fas fa-bolt"></i> FULL ELECTRICAL DESIGN REPORT</h2>
      <div style="display:flex;gap:8px;flex-wrap:wrap;flex-shrink:0">
        <button class="btn btn-secondary btn-sm" onclick="window.print()"><i class="fas fa-print"></i> Print</button>
        <button class="btn btn-amber btn-sm" onclick="exportSVG()"><i class="fas fa-download"></i> SVG</button>
        <button class="btn btn-primary btn-sm" onclick="goStep(1)"><i class="fas fa-edit"></i> Edit</button>
      </div>
    </div>
    ...
    <div class="tab-panel active" id="tab-circuits">
      <div class="sc">
        <div class="sc-hdr"><div class="ic"><i class="fas fa-bolt"></i></div><h2>CIRCUIT SCHEDULE</h2></div>
      ...
      <!-- Supply Summary -->
      <div class="sc">
        <div class="sc-hdr"><div class="ic"><i class="fas fa-plug"></i></div><h2>MAIN SUPPLY SUMMARY</h2></div>
        <div class="sc-body">
          <div class="fg fg-3">
            <div class="ssb">
    ...
    <div class="tab-panel" id="tab-materials">
      <div class="sc">
        <div class="sc-hdr"><div class="ic"><i class="fas fa-clipboard-list"></i></div><h2>COMPLETE PROFESSIONAL SHOPPING LIST</h2>
          <button class="btn btn-secondary btn-sm" style="margin-left:auto" onclick="copyMaterialList()"><i class="fas fa-copy"></i> Copy List</button>
        </div>
        <div class="sc-body">
...
function generateSLD(R) {
  ...
  // Defs
  svg += `<defs>
    <marker id="arr" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#0088CC"/></marker>
  ...
  const box = (x, cy, w, h, fill, stroke, label1, label2 = '', label3 = '') => {
    ...
    if (label2) s += `<text x="${x}" y="${cy + 8}" text-anchor="middle" fill="#0088CC" font-size="9">${label2}</text>`;
    if (label3) s += `<text x="${x}" y="${cy + 20}" text-anchor="middle" fill="#0088CC" font-size="9">${label3}</text>`;
    return s;
  };
  const line = (x1, y1, x2, y2, color = '#0088CC', dashed = false, w = 1.5) => {
    return `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${color}" stroke-width="${w}" ${dashed ? 'stroke-dasharray="4,3"' : ''} marker-end="url(#arr)"/>`;
  };
  const linePlain = (x1, y1, x2, y2, color = '#0088CC', w = 1.5) => {
    return `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${color}" stroke-width="${w}"/>`;
  };
  const label = (x, y, txt, col = '#0088CC', sz = 9) => `<text x="${x}" y="${y}" fill="${col}" font-size="${sz}">${txt}</text>`;
  ...
  // METER
  svg += box(cx, y + 22, 110, 36, '#0a1a0a', '#00cc66', 'METER', `Bi-directional digital`);
  y += 44;
  svg += linePlain(cx, y, cx, y + 16, '#00cc66');
  y += 16;

  // MAIN ISOLATOR
  svg += box(cx, y + 20, 110, 34, '#0a1020', '#0088CC', 'MAIN ISOLATOR', `${R.mainBreakerA}A DP Switch`);
  svg += label(cx + 60, y + 20, '← EMERGENCY OFF', '#ff888888', 8);
  y += 40;
  svg += linePlain(cx, y, cx, y + 16, '#0088CC');
  y += 16;
  ...
  svg += label(cx + 65, y + 22, `${R.mainCableSize}mm²`, '#0088CC', 9);
  y += 44;
  ...
  // Distribution bus
  const busY = y + 20;
  svg += `<rect x="${cx - Math.max(140, cols.length * colW / 2 + 20)}" y="${busY}" width="${Math.max(280, cols.length * colW + 40)}" height="14" rx="2" fill="#1a2a4a" stroke="#0088CC" stroke-width="2"/>`;
  svg += `<text x="${cx}" y="${busY + 10}" text-anchor="middle" fill="#0088CC" font-size="9" font-weight="bold">DISTRIBUTION BUS ${V}V</text>`;
  svg += linePlain(cx, y, cx, busY, '#0088CC');
  y = busY + 14;

  // Draw circuit columns
  const groupNames = ['LIGHTING', 'SOCKETS', 'HEAVY', 'SHOWER', 'AC'];
  const groupColors = ['#0088CC', '#ffb300', '#ff4444', '#a855f7', '#00ff88'];
  const groupTypes = ['lighting', 'socket', 'heavy', 'shower', 'ac'];
  let colX = cx - Math.max(110, cols.length * colW / 2 - colW / 2) + 30;

  cols.forEach((grp, gi) => {
    const color = groupColors[gi] || '#0088CC';
    let cy2 = y;
  ...
  svg += `<text x="28" y="36" fill="#0088CC" font-size="9" font-weight="bold">LEGEND</text>`;
  const lgItems = [['#0088CC', 'Lighting Circuit'], ['#ffb300', 'Socket Circuit'], ['#ff4444', 'Heavy Load'], ['#a855f7', 'Shower/Wet'], ['#00ff88', 'AC / Split']];
  lgItems.forEach(([c, t], i) => {
    svg += `<rect x="28" y="${44 + i * 13}" width="10" height="6" fill="${c}" rx="1"/>`;
    svg += `<text x="44" y="${51 + i * 13}" fill="#0088CC" font-size="8">${t}</text>`;
  });
...
function generateDBLayout(R) {
  ...
  svg += `<text x="${px + (dbW + 40) / 2}" y="${py - 12}" text-anchor="middle" fill="#0088CC" font-size="11" font-weight="bold" font-family="'Orbitron',monospace">DISTRIBUTION BOARD — ${R.dbWays} WAY</text>`;

  const breaker = (x, y, label1, label2, type, rating) => {
    const colors = { main: '#4a1a1a', rcd: '#0a2a0a', rcbo: '#1a0a2a', spd: '#2a1a00', mcb: '#0a0a2a' };
    const strs = { main: '#ff4444', rcd: '#00ff88', rcbo: '#a855f7', spd: '#ffb300', mcb: '#0088CC' };
  ...
  svg += `<text x="${lx + 8}" y="${py + 18}" fill="#0088CC" font-size="10" font-weight="bold">DB LEGEND</text>`;
  const lgItems = [
  ...
  lgItems.forEach(([bg, stroke, lbl], i) => {
    ...
    svg += `<text x="${lx + 46}" y="${py + 41 + i * 28}" fill="#0088CC" font-size="9">${lbl}</text>`;
  });

  // Stats
  svg += `<rect x="${lx}" y="${py + 190}" width="140" height="130" rx="4" fill="#0a1020" stroke="#1e3a5c"/>`;
  svg += `<text x="${lx + 8}" y="${py + 208}" fill="#0088CC" font-size="9" font-weight="bold">DB SUMMARY</text>`;
  const stats = [
  ...
  stats.forEach(([t], i) => {
    svg += `<text x="${lx + 8}" y="${py + 224 + i * 16}" fill="#0088CC" font-size="8">${t}</text>`;
  });
...
function generateEarthingDiagram(R) {
  ...
  const txt = (x, y, t, c = '#0088CC', sz = 9, anc = 'start') => `<text x="${x}" y="${y}" fill="${c}" font-size="${sz}" text-anchor="${anc}">${t}</text>`;
  ...
  // Main DB box
  svg += rect(300, 60, 160, 60, '#0a1628', '#0088CC', 4);
  svg += txt(380, 85, 'DISTRIBUTION BOARD', '#0088CC', 9, 'middle');
  svg += txt(380, 100, `${R.mainBreakerA}A Main | ${R.dbWays}-way`, '#0088CC', 8, 'middle');
  ...
  svg += txt(610, svgH - 110, 'EARTHING SYSTEM', '#0088CC', 9);
  svg += txt(610, svgH - 95, `Type: ${R.earthing}`, '#0088CC', 9);
  svg += txt(610, svgH - 80, `Rod length: ${R.earthRodLength}m`, '#0088CC', 8);
  svg += txt(610, svgH - 65, `Earth cable: ${R.earthCableSize}mm²`, '#0088CC', 8);
...
function generateProtectionDiagram(R) {
  ...
  const levels = [
    ...
    { label: 'CIRCUIT PROTECTION', sub: 'Individual circuit breakers', color: '#0088CC', y: 390, devices: [`${R.allCircuits.length} circuits: MCBs + ${R.rcboCount} RCBOs`] },
    { label: 'LOAD (FINAL CIRCUIT)', sub: 'Point of use — appliances & outlets', color: '#0088CC', y: 460, devices: ['Sockets, luminaires, appliances'] },
  ];
  ...
  levels.forEach((lv, i) => {
    ...
    svg += `<text x="${bx + 10}" y="${by + 30}" fill="#0088CC" font-size="8">${lv.sub}</text>`;
  ...
  // Title
  svg += `<text x="400" y="18" fill="#0088CC" font-size="11" font-weight="bold" text-anchor="middle" font-family="'Orbitron',monospace">PROTECTION COORDINATION HIERARCHY — IEC 60364</text>`;
...
window.addEventListener('load', () => {
  function hideOverlay() {
    ...
    setTimeout(() => { if (ov) { ov.classList.add('gone'); ov.style.display = 'none'; } }, 450);
  }
